...
def log_in(
    ...
    *,
    ...
    """
    existing_valid_token = app.auth_context.get_access_token()
    ...
    if app.config.client_id and app.config.client_secret:
        access_token = app.oidc_context.authenticate_with_client_credentials(
            (app.config.client_id, app.config.client_secret)
        )
    else:
        access_token = app.oidc_context.authenticate_device()

    if app.config.store_access_token:
        app.auth_context.save_access_token(access_token)
...
def get_access_token(app: ArnicaApp) -> str | None:
    ...
    """
    return app.auth_context.get_access_token()
